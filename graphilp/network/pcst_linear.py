...
def create_model(G, forced_terminals=[], weight='weight', prize='prize',
    ...
    """
    m.setObjective(quicksum([edge_var * G.G.edges[edge][weight] for edge, edge_var in edges.items()])
                + sum(G.G.nodes[node].get(prize, 0) for node in nodes)
                - quicksum([G.G.nodes[node].get(prize, 0) * node_var for node, node_var in nodes.items()]),
                   GRB.MINIMIZE)
    """
