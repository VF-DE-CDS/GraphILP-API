...
edge_dict = None
rev_edge_dict = None
def createModel(G, roots, cycleBasis: bool = False, nodeColoring: bool = False):    
    ...
    m.setObjective(
        ...
        - gurobipy.quicksum([edge_list[u,v]['weight'] * m.getVarByName("edge_" + str(u) + "_" + str(v)) for (u, v) in edge_list])\
        - gurobipy.quicksum([edge_list[u,v]['weight'] * m.getVarByName("edge_" + str(v) + "_" + str(u)) for (u, v) in edge_list]),\
        GRB.MAXIMIZE)
    ...
    # Enforce a root
    m.addConstr(m.getVarByName("node_" + str(roots)) == 1)
