...
def create_model(G, forced_terminals, weight='weight', prize='prize', warmstart=[]):
    ...
    edge_flow = m.addVars(G.G.edges(), vtype=GRB.INTEGER, lb=-T+1, ub=T-1)
    ...
    for node in G.G.nodes():
        if node not in terminals:
            c = m.addConstr(quicksum(edge_flow[e] for e in G.G.edges(node)) - quicksum(edge_flow[e] for e in G.G.in_edges(node)) == 0)
